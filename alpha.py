...
class HypocycloidAnimation:
    def __init__(self, points=3, cycles=1, points_per_revolution=256):
        self.init_plot()
        self.init_objects()

    def init_plot(self):
        self.fig, self.ax = plt.subplots()
        plt.axis('off')  # TODO - add self?
    ...
    def init_objects(self):
        ...
        self.big_circle, = self.ax.plot([], [], 'b', linewidth=2)
        # big circle:
        # theta = np.linspace(0, 2*np.pi*nloops, resolution*nloops+1)
        # x = np.cos(theta)
        # y = np.sin(theta)
        # self.big_circle, = self.ax.plot(x, y, 'b', linewidth=2)
        # self.dot_circle, = self.ax.plot(x*(1-1/num), y*(1-1/num), 'k--', linewidth=1.5)
        # self.dot, = self.ax.plot([], [], 'ko', ms=10)
        # self.hypocycloid, = self.ax.plot([], [], 'r-', linewidth=3.3)
        # self.small_circle, = self.ax.plot([], [], 'k-', linewidth=2)
        # self.line, = self.ax.plot([], [], 'k-', linewidth=2)

    def frame_update(self, step=8):
        '''Updates the plot paramenters for the next frame and then yields.
        '''
        for i in range(80):
            yield
        '''
