...
def frame_generator(step=8):
    ...
    for i in range(0, len(theta), step):
        ...
        hypo_rotate = hypo_rotate * np.conj(circle[i])
        hypocycloid_a.set_data(hypo_rotate.real, hypo_rotate.imag)
        hypo_b = hypo_outer * np.conj(circle[i])
        hypocycloid_b.set_data(hypo_b.real, hypo_b.imag)
...
ani.save('hypo_vase2.gif', writer='imagemagick', fps=10, dpi=75)
# plt.show()
"""
